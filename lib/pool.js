...
var pool = module.exports = function pool(options, authorizeFn){
            ...
            */
            if (!isValidBlock)
                ...
                SubmitBlock(blockHex, function(){
                    CheckBlockAccepted(shareData, function(isAccepted, tx){
    ...
    function CheckBlockAccepted(shareData, callback){
        //setTimeout(function(){
            _this.daemon.cmd('getblockhash',
                [shareData.height],
                function(blockHash){
                    if (blockHash.length <= 0){
                        callback(false);
                    }
                    var hash = blockHash[0].response;
                    emitLog('BlockHash at height ' + shareData.height + ': ' + hash);
                    _this.daemon.cmd('getblock',
                        [hash],
                        function(results){
                            var numNonce = parseInt(shareData.nonce, 16);
                            var validResults = results.filter(function(result){
                                if (result.response) {
                                    emitLog('Block nonce: ' + result.response.nonce + ' . Mined nonce: ' + numNonce);
                                }
                                return result.response && (result.response.nonce === numNonce)
                            });
