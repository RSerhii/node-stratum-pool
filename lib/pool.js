    ...
    function CheckBlockAccepted(shareData, callback){
        //setTimeout(function(){
            _this.daemon.cmd('getblockhash',
                [shareData.height],
                function(blockHash){
                    var hash = JSON.stringify(blockHash);
                    ...
                    _this.daemon.cmd('getblock',
                        ...
                        function(results){
                            var validResults = results.filter(function(result){
                                if (result.response) {
                                    emitLog('Block nonce: ' + result.response.nonce + ' . Mined nonce: ' + shareData.nonce);
                                }
                                return result.response && (result.response.nonce === shareData.nonce)
                            });
